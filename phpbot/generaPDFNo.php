...
{    $this->SetMargins(35, 25, 30);
	...
	$this->Cell(50,10);
	$this->Cell(0,10,'Reporte de Mensajes sin responder');
	$this->Ln(15);
...
$pdf->SetFont('Arial', '', 8);
//CONSULTA
$noSolucion = mysqli_query($conexion,"SELECT * FROM no_answer");
$item = 0;

while($usuarios = mysqli_fetch_array($noSolucion)){
    $cuerpo = $usuarios['body'];
    $cuerpodeco = utf8_decode($cuerpo);

	$item = $item+1;
    $pdf->Cell(10, 8, $item, 0);
	$pdf->Cell(60, 8, $usuarios['email'], 0);
